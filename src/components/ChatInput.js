...
const ChatInput = ({
  ...
  const [showQuickResponseDropdown, setShowQuickResponseDropdown] =
    useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        quickResponseRef.current &&
        !quickResponseRef.current.contains(event.target)
      ) {
        setShowQuickResponseDropdown(false);
      }
      if (
        knowledgeBaseRef.current &&
        !knowledgeBaseRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    const updateMessagesPadding = () => {
      if (textareaRef.current) {
        const inputContainer = textareaRef.current.closest(
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      alert(
        currentLanguage === "zh" ? "图片处理失败" : "Failed to process image"
      );
    }
  };
  ...
  const readDocumentContent = async (file) => {
    try {
      ...
      if (text.length > maxChars) {
        alert(
          currentLanguage === "zh"
            ? `文档内容过长（${text.length}字符），超过3000字符限制，请添加到知识库中处理`
            : `Document content too long (${text.length} characters), exceeds 3000 character limit, please add to knowledge base`
        );
      ...
      if (error.message === "Unsupported file type") {
        alert(
          currentLanguage === "zh"
            ? "不支持的文件格式，请上传文本文件、PDF或Word文档"
            : "Unsupported file format, please upload text files, PDF or Word documents"
        );
      }
      throw error;
    }
  ...
  return (
    <div className={`chat-input-container ${className}`}>
      <form onSubmit={handleSubmit} className="chat-input-form">
        <div
          className={`input-wrapper-clean ${
            isStreaming && streamingState !== "unknown"
            ...
            <textarea
              ...
              placeholder={
                isStreaming
                  ...
                  : placeholder ||
                    (uploadedFile
                      ? t("typeMessageWithFile", currentLanguage)
                      : t("typeMessage", currentLanguage))
              }
              disabled={disabled}
              rows={1}
              ref={textareaRef}
            ...
            {isStreaming ? (
              ...
              <button
                ...
                className={`send-button ${
                  message.trim() || uploadedFile ? "has-content" : ""
                } ${
                  isStreaming && streamingState !== "unknown"
                    ? `streaming streaming-${streamingState}`
                    : ""
              ...
              >
                {message.trim() || uploadedFile ? (
                  ...
                  >
                    ...
                    <path
                      fill="currentColor"
                      d="M11 16h2v-4.2l1.6 1.6L16 12l-4-4l-4 4l1.4 1.4l1.6-1.6zm1 6q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22"
                    />
                  </svg>
                ) : (
                  <svg
            ...
            <div className="toolbar-left">
                ...
                {showUploadDropdown && (
                  <div className="upload-dropdown">
                    ...
                    >
                      ...
                      <span>
                        {currentLanguage === "zh" ? "上传图片" : "Upload Image"}
                      </span>
                    </button>
                  </div>
                )}
              ...
              {supportsResponseModes && (
                <div className="response-mode-slider-container">
                  <div
                    className={`response-mode-slider ${
                    ...
                    onClick={() => {
                      const newMode =
                        responseMode === "normal" ? "thinking" : "normal";
                ...
                <div className="knowledge-base-dropdown" ref={knowledgeBaseRef}>
                  <button
                    ...
                    className={`knowledge-base-btn ${
                      selectedDocuments.length > 0 ? "active" : ""
                    }`}
                    onClick={() => {
