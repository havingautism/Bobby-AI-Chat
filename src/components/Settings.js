...
// 已移除未使用的 ModelOption 组件

// 已移除未使用的模型分类组件，避免未使用告警
...
const Settings = ({ isOpen, onClose, onModelChange }) => {
  ...
  // 取消独立测试消息展示，统一用按钮状态提示
  const [showModelDropdown, setShowModelDropdown] = useState(false);
  const [dropdownPosition, setDropdownPosition] = useState("bottom");
  const [storageInfo, setStorageInfo] = useState(null);
  const [currentStorageType] = useState(() => storageAdapter.getStorageType());
  ...
  const getConversationCount = () => {
    ...
    if (isTauriEnvironment()) {
      ...
      return (
        storageInfo.conversationCount || storageInfo.conversations?.count || 0
      );
    } else {
  ...
  useEffect(() => {
    if (isOpen) {
      const loadData = async () => {
        ...
        // 过滤掉温度与Top P（已迁移到角色管理）
        setConfig((prev) => ({
          baseURL: currentConfig.baseURL ?? prev.baseURL,
          apiKey: currentConfig.apiKey ?? prev.apiKey,
          model: modelToUse,
          maxTokens:
            currentConfig.maxTokens !== undefined
              ? currentConfig.maxTokens
              : prev.maxTokens,
          thinkingBudget:
            currentConfig.thinkingBudget !== undefined
              ? currentConfig.thinkingBudget
              : prev.thinkingBudget,
        }));
