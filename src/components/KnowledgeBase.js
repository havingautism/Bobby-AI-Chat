...
const KnowledgeBase = ({ isOpen, onClose }) => {
  ...
  const parseFileName = (fileName) => {
    if (!fileName) return { name: "document", extension: "" };
  ...
  const handleAddText = async (textData) => {
    try {
      ...
      showSuccess(
        ...
        currentLanguage === "zh"
          ? "文本内容已成功添加到知识库"
          : "Text content has been successfully added to the knowledge base"
      );
  ...
  const testDocumentAdd = async () => {
    try {
      ...
      const testDocument = {
        title: "测试文档 - 中文",
        content:
          "这是一个测试文档，用于验证中文文档的添加功能。文档包含中文内容，应该被正确识别并存储到对应的中文知识库中。",
  ...
  const debugDatabase = async () => {
    try {
      ...
      // 调用知识库的调试功能
      const debugInfo =
        await knowledgeBaseManager.knowledgeBase.debugDatabaseInfo();
      ...
      if (debugInfo) {
        ...
        debugInfo.collections.forEach((collection, index) => {
          console.log(
            `[${index + 1}] 集合: ${collection.name} (${collection.id})`
          );
...
`
  ...
  const runSemanticSearchTests = async () => {
    if (statistics.vectorCount === 0) {
      alert(
        currentLanguage === "zh"
          ? "请先生成向量嵌入"
          : "Please generate vector embeddings first"
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `📈 当前统计: 文档 ${statistics.documentCount} 个, 向量 ${statistics.vectorCount} 个\n`
      );
      ...
      for (let i = 0; i < testQueries.length; i++) {
        ...
        try {
          ...
          // 执行向量搜索
          const results = await knowledgeBaseManager.searchSQLite(
            testCase.query,
            5,
            0.3,
            false
          ...
          if (results.length > 0) {
            ...
            console.log(
              `   - 平均相似度: ${
                avgScore !== undefined ? avgScore.toFixed(4) : "N/A"
              }`
            );
            console.log(
              `   - 最高相似度: ${
                maxScore !== undefined ? maxScore.toFixed(4) : "N/A"
              }`
            );
            console.log(
              `   - 最低相似度: ${
                minScore !== undefined ? minScore.toFixed(4) : "N/A"
              }`
            );
  ...
  const removeTest = (queryToRemove) => {
    setTestQueries((prev) =>
      prev.filter((test) => test.query !== queryToRemove)
    );
    // 同时删除对应的测试结果
    setTestResults((prev) => {
  ...
  const handleGenerateVectors = async () => {
    try {
      ...
      alert(
        currentLanguage === "zh" ? "生成向量失败" : "Failed to generate vectors"
      );
    }
  };
  ...
  const debugVectorGeneration = async () => {
    try {
      ...
      alert(
        currentLanguage === "zh"
          ? "向量生成调试完成，请查看控制台"
          : "Vector generation debug completed, check console"
      );
    } catch (error) {
  ...
  const cleanupDuplicateDocuments = async () => {
    try {
      ...
      // 找出重复的文档组
      const duplicates = Object.values(docGroups).filter(
      ...
      alert(
        currentLanguage === "zh"
          ? `已清理 ${deletedCount} 个重复文档`
          : `Cleaned up ${deletedCount} duplicate documents`
      );
    } catch (error) {
    ...
    setLoadingModal({
      ...
      steps: [
        ...
        },
        {
          ...
          description:
            currentLanguage === "zh" ? "上传完成" : "Upload completed",
        },
      ],
      currentStep: 0,
      showCancel: true,
      onCancel: () => {
        ...
        showWarning(
          ...
          currentLanguage === "zh"
            ? "文件上传操作已取消"
            : "File upload operation was cancelled"
        );
      },
    });
    ...
    try {
        ...
        try {
          ...
          setLoadingModal((s) => ({
            ...
            message:
              currentLanguage === "zh"
                ? `存储到知识库: ${file.name}`
                : `Storing to knowledge base: ${file.name}`,
          }));
      ...
      if (hasFailures) {
        ...
        setSuccessModal({
          ...
          details: [
            {
              label: currentLanguage === "zh" ? "上传成功" : "Success",
              value: `${uploadedFiles.length} 个文件`,
            },
            {
              label: currentLanguage === "zh" ? "上传失败" : "Failed",
              value: `${failedFiles.length} 个文件`,
            },
            {
              label: currentLanguage === "zh" ? "失败原因" : "Failure Reasons",
              value: failedFiles
                .map((f) => `${f.file.name}: ${f.error.message || "未知错误"}`)
                .join("; "),
            },
            {
              label: currentLanguage === "zh" ? "完成时间" : "Completed",
        ...
        setSuccessModal({
          ...
          details: [
            ...
            {
              label: currentLanguage === "zh" ? "总大小" : "Total Size",
      ...
      showError(
        ...
        currentLanguage === "zh"
          ? "文件上传失败，请检查文件格式和网络连接"
          : "File upload failed, please check file format and network connection",
        {
          actions: [
            {
        ...
        steps: [
          {
            ...
            title:
              currentLanguage === "zh"
                ? "移除向量数据"
                : "Removing Vector Data",
            description:
              currentLanguage === "zh"
                ? "从向量数据库中删除文档向量"
                : "Deleting document vectors from vector database",
            status: "pending",
          },
          {
            id: "remove_metadata",
            title:
              currentLanguage === "zh" ? "清理元数据" : "Cleaning Metadata",
            description:
              currentLanguage === "zh"
                ? "删除文档元数据和记录"
                : "Removing document metadata and records",
            status: "pending",
          },
          {
            id: "update_index",
                  ...
                  <div className="stat-card">
                    ...
                    <div className="stat-content">
                      ...
                      <span className="stat-label">
                        {currentLanguage === "zh" ? "文档数量" : "Documents"}
                      </span>
                    </div>
                  </div>
                  <div className="stat-card">
                    ...
                    <div className="stat-content">
                      ...
                      <span className="stat-label">
                        {currentLanguage === "zh" ? "向量数量" : "Vectors"}
                      </span>
                    </div>
                  </div>
                  <div className="stat-card">
                    ...
                    <div className="stat-content">
                      ...
                      <span className="stat-label">
                        {currentLanguage === "zh" ? "总大小" : "Total Size"}
                      </span>
                    </div>
                  </div>
                </>
            ...
            {activeTab === "documents" && (
              <div className="tab-content">
                <div className="documents-header">
                  <h3>
                    {currentLanguage === "zh" ? "文档列表" : "Document List"}
                  </h3>
                  <div className="header-actions">
                    {/* <button
                ...
                <div className="document-grid">
                  {filteredDocuments.length === 0 ? (
                    ...
                    filteredDocuments.map((doc, index) => {
                      const fileName = doc.title || doc.fileName || "document";
                ...
                <div className="test-section">
                  ...
                  {showCustomTestForm && (
                    <div className="custom-test-form">
                      ...
                      <div className="form-group">
                        <label>
                          {currentLanguage === "zh" ? "测试查询" : "Test Query"}
                        </label>
                        <input
                          type="text"
                  ...
                  <div className="test-cases-section">
                    <h4>
                      {currentLanguage === "zh" ? "测试用例列表" : "Test Cases"}
                    </h4>
                    <div className="test-cases-list">
                      {testQueries.map((testCase, index) => (
                        <div key={index} className="test-case-item">
                          <div className="test-case-info">
                            <div className="test-case-query">
                              "{testCase.query}"
                            </div>
                  ...
                  {statistics.vectorCount === 0 && (
                    <div className="test-warning">
                      ...
                      <p>
                        {currentLanguage === "zh"
                          ? "请先生成向量嵌入才能进行测试"
                          : "Please generate vector embeddings first to run tests"}
                      </p>
                    </div>
                  )}
