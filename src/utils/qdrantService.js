...
class QdrantService {
  ...
  generateUuidV4() {
    if (typeof crypto !== "undefined" && crypto.getRandomValues) {
      ...
      const bytesToHex = Array.from(buf).map((b) =>
  ...
  async ensureCollection(collectionName = null, vectorSize = 384) {
    ...
    try {
      ...
      const response = await this.fetchWithTimeout(
        `http://localhost:6333/collections/${targetCollection}`
      );
      if (response.ok) {
        ...
        if (currentVectorSize === vectorSize) {
          console.log(
            `✅ Qdrant集合 ${targetCollection} 已存在，维度正确 (${vectorSize})`
          );
          return true; // 集合已存在且维度正确
        } else {
          console.log(
            `⚠️ Qdrant集合 ${targetCollection} 存在但维度不匹配 (当前: ${currentVectorSize}, 需要: ${vectorSize})，重新创建...`
          );

          // 删除现有集合
          const deleteResponse = await this.fetchWithTimeout(
            `http://localhost:6333/collections/${targetCollection}`,
            {
              method: "DELETE",
            }
          ...
          if (deleteResponse.ok) {
            ...
            console.warn(
              `⚠️ 删除旧集合 ${targetCollection} 失败，继续创建新集合`
            );
          }

          // 删除后重新创建集合
          console.log(
            `🔧 重新创建 Qdrant集合 ${targetCollection} (维度: ${vectorSize})...`
          );
          ...
          if (createResponse.ok) {
            ...
            console.error(
              `❌ Qdrant集合 ${targetCollection} 重新创建失败:`,
      ...
      if (response.status === 404) {
        ...
        if (createResponse.ok) {
          ...
          const errorText = await createResponse.text();
          console.error(
            `❌ Qdrant集合 ${targetCollection} 创建失败:`,
            createResponse.status,
            createResponse.statusText
          );
          console.error(`❌ 错误详情:`, errorText);
  ...
  async upsertPoints(points, targetCollection = null) {
    ...
    try {
      // 检测向量维度
      const vectorSize =
        points.length > 0 && points[0].vector ? points[0].vector.length : 384;
      ...
      const response = await this.fetchWithTimeout(
        `http://localhost:6333/collections/${targetCollection}/points`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        console.error(
          "请求数据:",
          JSON.stringify({ points: qdrantPoints }, null, 2)
        );
        return false;
      }
    } catch (error) {
  ...
  async search(request, targetCollection = null) {
    ...
    try {
      ...
      console.log(
        "🔍 请求体中的vector字段:",
        requestBody.vector
          ? `存在，长度: ${requestBody.vector.length}`
          : "不存在"
      );

      const response = await this.fetchWithTimeout(
        `http://localhost:6333/collections/${targetCollection}/points/search`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async deletePoints(pointIds, targetCollection = null) {
    try {
      console.log(
        `🗑️ 在集合 ${targetCollection} 中删除 ${pointIds.length} 个向量点:`,
        pointIds
      );

      const response = await fetch(
        `http://localhost:6333/collections/${targetCollection}/points/delete`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async selectCollectionByLanguage(text) {
    try {
      ...
      console.log(
        `🎯 语言检测结果: ${config.detectedLanguage}, 选择集合: ${config.collection}`
      );
  ...
  async searchAllCollections(query, limit = 10, threshold = 0.01) {
    try {
      ...
      console.log(
        `🎯 主要搜索语言: ${primaryConfig.detectedLanguage}, 模型: ${primaryConfig.model}`
      );

      // 生成查询向量
      const queryResult = await embeddingService.generateEmbeddings(
        [query],
        primaryConfig.model
      );
      ...
      console.log(
        `📊 使用${primaryConfig.model}模型生成查询向量 (${queryVector.length}维)`
      );

      // 在所有集合中搜索
      const searchPromises = Object.entries(this.collections).map(
        async ([lang, targetCollection]) => {
          try {
            const searchRequest = {
              vector: queryVector,
  ...
  async clearCollection() {
    ...
    try {
      ...
      const scrollResponse = await fetch(
        `http://localhost:6333/collections/${this.defaultCollection}/points/scroll`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const deleteResponse = await fetch(
        `http://localhost:6333/collections/${this.defaultCollection}/points/delete`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      ...
      const points = embeddings.map((embeddingData, index) => {
        ...
        if (Array.isArray(embeddingData.embedding)) {
          ...
          Array.isArray(embeddingData.embedding.embedding)
        ) {
        ...
        // 使用真正的分块索引，而不是数组索引
        const chunkIndex =
          embeddingData.chunkIndex !== undefined
      ...
      if (success) {
        console.log(
          `✅ 文档 ${documentId} 的向量已存储到Qdrant，共 ${points.length} 个向量`
        );
      }
  ...
  async searchDocuments(query, limit = 10, threshold = 0.01) {
    ...
    try {
      ...
      console.log(
        `📊 使用${queryResult.model}模型生成查询向量 (${queryResult.dimensions}维)`
      );
      ...
      console.log(
        "🔍 查询向量长度:",
        queryVector ? queryVector.length : "undefined"
      );
      ...
      const results = (
        ...
        return {
          ...
          fileName:
            result.payload?.file_name || result.payload?.filename || null,
  ...
  async deleteDocumentVectors(documentId) {
    try {
      ...
      console.log(
        `🔍 使用scroll API获取所有向量点，然后过滤文档 ${documentId}...`
      );
      ...
      if (success) {
        console.log(
          `✅ 成功删除文档 ${documentId} 的 ${pointIds.length} 个向量`
        );
  ...
  async optimizeCollection() {
    ...
    try {
      ...
      try {
        const response = await fetch(
          `http://localhost:6333/collections/${this.defaultCollection}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
    ...
    try {
      ...
      try {
        const snapshotResponse = await fetch(
          `http://localhost:6333/collections/${this.defaultCollection}/snapshots`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  async getStatistics() {
    ...
    try {
      ...
      if (totalVectorsCount === 0) {
        const defaultCollectionInfo = await this.getCollectionInfo(
