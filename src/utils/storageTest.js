...
class StorageTest {
  ...
  async testBasicStorage() {
    ...
    try {
      ...
      if (loadedValue === 'test-value') {
        ...
        this.addResult('基本设置存储', 'FAIL', `期望: test-value, 实际: ${loadedValue}`);
      }
      ...
      const foundConversation = conversations.find(c => c.id === 'test-conv-1');
  ...
  async testStorageSwitching() {
    ...
    try {
      ...
      await storageAdapter.saveConversation(testConversation);
      await storageAdapter.saveSetting('switch-test-setting', 'switch-test-value');

      // 测试切换到SQLite
      if (currentType !== 'sqlite') {
        try {
          await storageAdapter.switchToSQLite();
          const newType = storageAdapter.getStorageType();
          
          if (newType === 'sqlite') {
            // 验证数据是否迁移成功
            const conversations = await storageAdapter.loadChatHistory();
            const foundConversation = conversations.find(c => c.id === 'switch-test-conv');
            const settingValue = await storageAdapter.loadSetting('switch-test-setting');
            
            if (foundConversation && settingValue === 'switch-test-value') {
              this.addResult('切换到SQLite', 'PASS');
            } else {
              this.addResult('切换到SQLite', 'FAIL', '数据迁移不完整');
            }
          } else {
            this.addResult('切换到SQLite', 'FAIL', `期望: sqlite, 实际: ${newType}`);
          }
        } catch (error) {
          this.addResult('切换到SQLite', 'ERROR', error.message);
        }
      }
      ...
      if (storageAdapter.getStorageType() === 'sqlite') {
        try {
          ...
          if (newType === 'tauri') {
            ...
            if (foundConversation && settingValue === 'switch-test-value') {
              this.addResult('切换到JSON', 'PASS');
            } else {
              this.addResult('切换到JSON', 'FAIL', '数据迁移不完整');
            }
          } else {
            this.addResult('切换到JSON', 'FAIL', `期望: tauri, 实际: ${newType}`);
          }
        } catch (error) {
          this.addResult('切换到JSON', 'ERROR', error.message);
        }
      }
  ...
  async testKnowledgeBase() {
    ...
    try {
      ...
      const testDocument = {
        id: 'test-doc-1',
        title: '测试文档',
        content: '这是一个测试文档，用于验证知识库功能。它包含了一些中文内容，用于测试向量搜索功能。',
        sourceType: 'text',
        metadata: {
          author: 'test',
          category: 'test'
      ...
      // 测试搜索功能
      const searchResults = await knowledgeBaseManager.search('测试文档', { limit: 5 });
    ...
    try {
      ...
      if (storageAdapter.getStorageType() === 'sqlite') {
        ...
        if (migrationResult) {
          // 验证迁移结果
          const conversations = await storageAdapter.loadChatHistory();
          const foundConversation = conversations.find(c => c.id === 'migration-test-1');
          const settingValue = await storageAdapter.loadSetting('migration-test-setting');
  ...
  printResults() {
    ...
    console.log(`环境: ${storageAdapter.isTauriEnvironment() ? 'Tauri' : 'Web'}`);
    console.log('');
    ...
    const successRate = ((statusCounts.PASS / this.testResults.length) * 100).toFixed(1);
