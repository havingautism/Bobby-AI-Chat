...
class KnowledgeBaseSQLiteVec {
  ...
  setEmbeddingConfig(
    ...
    console.log(
      `🔧 嵌入模型配置已更新: ${model} (${dimensions}维, ${taskType})`
    );
  }
  ...
  async initialize() {
    ...
    try {
      ...
      const dbHealth =
        systemStatus?.databaseHealth || systemStatus?.database_health || {};
      const ok =
        Boolean(dbHealth.knowledge_db) && Boolean(dbHealth.vec_extension);
      if (!ok) {
  ...
  async createCollection(
    ...
    try {
      console.log(
        `🔧 创建集合: name="${name}", description="${description}", model="${embeddingModel}", dimensions=${vectorDimensions}`
      );
  ...
  async generateDocumentEmbeddings(
    ...
    try {
      console.log(`🚀 generateDocumentEmbeddings 被调用，参数:`, {
        documentId,
        hasContent: !!content,
        model,
        collectionId,
      ...
      if (content) {
        ...
        console.log(
          `🎯 专家模型分离模式 - 检测语言: ${detectedLanguage}, 选择模型: ${finalModel}`
        );
        console.log(`🔍 详细模型配置:`, modelConfig);
      }

      console.log(
        `📋 即将调用后端API - documentId: ${documentId}, model: ${finalModel}, hasContent: ${!!content}, collectionId: ${collectionId}`
      );
      console.log(
        `🔍 详细参数检查 - collectionId类型: ${typeof collectionId}, collectionId值:`,
        collectionId
      );
      ...
      try {
        ...
        if (savedConfig) {
          ...
          console.log(
            "🔍 localStorage中的API密钥:",
            parsedConfig.apiKey
              ? `${parsedConfig.apiKey.substring(0, 10)}...`
              : "null"
          );
        }
      } catch (error) {
      ...
      console.log(
        "🔍 API密钥长度:",
        apiConfig.apiKey ? apiConfig.apiKey.length : "N/A"
      );
      console.log(
        "🔍 API密钥内容:",
        apiConfig.apiKey ? `${apiConfig.apiKey.substring(0, 10)}...` : "null"
      );
  ...
  async searchKnowledgeBase(
    ...
    try {
      ...
      const clampedThreshold = Math.min(
        0.75,
        Math.max(0.5, isNaN(numericThreshold) ? 0.5 : numericThreshold)
      );
      console.log(`🔍 搜索参数详情:`, {
        query: query,
      ...
      console.log("🔍 传递给后端的搜索参数:", {
        ...
        apiKey: searchParams.apiKey
          ? `${searchParams.apiKey.substring(0, 10)}...`
          : "null",
      });
