  ...
  async initialize() {
    try {
      ...
      if (stats.main_db && stats.knowledge_db && stats.vec_extension) {
        ...
        console.log("SQLite数据库初始化成功，使用Tauri后端SQLite + sqlite-vec系统");
      } else {
        throw new Error(`数据库初始化不完整: main_db=${stats.main_db}, knowledge_db=${stats.knowledge_db}, vec_extension=${stats.vec_extension}`);
      }
    } catch (error) {
  ...
  async getStorageInfo() {
    try {
      ...
      return {
        dbPath: this.dbPath,
        dbSize:
          dbSize > 0 ? `${(dbSize / (1024 * 1024)).toFixed(2)} MB` : "未知",
...
export const saveApiSessions =
  sqliteStorage.saveApiSessions.bind(sqliteStorage);
export const loadApiSessions =
  sqliteStorage.loadApiSessions.bind(sqliteStorage);
