...
async function diagnoseSearchIssue() {
  ...
  try {
    ...
    const documents = await window.__TAURI__.core.invoke('get_knowledge_documents', { 
    ...
    console.log('\n🔑 5. 检查API密钥配置:');
    try {
      const settings = await window.__TAURI__.core.invoke('get_all_settings');
